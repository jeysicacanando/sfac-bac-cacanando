...
if (isset($_POST['save'])) {
    ...
    $getAllUsername = mysqli_query($db, "SELECT username FROM tbl_admissions WHERE username = '$username' UNION ALL SELECT username FROM tbl_guidances WHERE username = '$username' UNION ALL SELECT username FROM tbl_guidances WHERE username = '$username' UNION ALL SELECT username FROM tbl_admins WHERE username = '$username' UNION ALL SELECT username FROM tbl_students WHERE username = '$username' UNION ALL SELECT username FROM tbl_presidents WHERE username = '$username' UNION ALL SELECT username FROM tbl_accounting WHERE username = '$username' UNION ALL SELECT username FROM tbl_super_admins WHERE username = '$username'") or die(mysqli_error($db));
    ...
    if ($check == 0) {
        ...
        if ($getID == $guidance_id || $check2 < 1) {
            $updateInfo = mysqli_query($db, " UPDATE tbl_guidances SET guidance_lastname='$lname',guidance_firstname='$fname', guidance_middlename='$mname', guidance_no = '$guidance_no', position = '$position', status = '$status', email='$email', username='$username', updated_by = '$updated_by', last_updated = CURRENT_TIMESTAMP WHERE guidance_id = '$guidance_id'") or die(mysqli_error($db));
